...
public class CubeController : MonoBehaviour {
	...
	private void initPieces(){
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            //   renderers[2].material.SetColor("_Color", Color.cyan);
            //centerPSpacing(centerP[i] , pistonLength);
        }

        //set the render color for every center of the cube
        Renderer[] renderCenter1_Blue = centerP[0].GetComponentsInChildren<Renderer>();
        renderCenter1_Blue[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[2, 4]));

        Renderer[] renderCenter2_Orange = centerP[1].GetComponentsInChildren<Renderer>();
        renderCenter2_Orange[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[5, 4]));

        Renderer[] renderCenter3_Green = centerP[2].GetComponentsInChildren<Renderer>();
        renderCenter3_Green[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[4, 4]));

        Renderer[] renderCenter4_Red = centerP[3].GetComponentsInChildren<Renderer>();
        renderCenter4_Red[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[0, 4]));

        Renderer[] renderCenter5_Yellow = centerP[4].GetComponentsInChildren<Renderer>();
        renderCenter5_Yellow[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[3, 4]));

        Renderer[] renderCenter6_White = centerP[5].GetComponentsInChildren<Renderer>();
        renderCenter6_White[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[1, 4]));


 /*       //BACK-UP EXAMPLE
        Renderer[] renderCenter1_Blue = centerP[0].GetComponentsInChildren<Renderer>();
        renderCenter1_Blue[1].material.SetColor("_Color", Color.blue);

        Renderer[] renderCenter2_Orange = centerP[1].GetComponentsInChildren<Renderer>();
        renderCenter2_Orange[1].material.SetColor("_Color", new Color(1, 0.27f, 0));

        Renderer[] renderCenter3_Green = centerP[2].GetComponentsInChildren<Renderer>();
        renderCenter3_Green[1].material.SetColor("_Color", Color.green);

        Renderer[] renderCenter4_Red = centerP[3].GetComponentsInChildren<Renderer>();
        renderCenter4_Red[1].material.SetColor("_Color", Color.red);

        Renderer[] renderCenter5_Yellow = centerP[4].GetComponentsInChildren<Renderer>();
        renderCenter5_Yellow[1].material.SetColor("_Color", Color.yellow);

        Renderer[] renderCenter6_White = centerP[5].GetComponentsInChildren<Renderer>();
        renderCenter6_White[1].material.SetColor("_Color", Color.white);
        */
        ...
        //set the color for every Corner of the cube
         Renderer[] renderCorner1_BWO = cornerP[0].GetComponentsInChildren<Renderer>();
         renderCorner1_BWO[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[2, 0]));
         renderCorner1_BWO[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[1, 0]));
         renderCorner1_BWO[3].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[5, 2]));

         Renderer[] renderCorner2_RWB = cornerP[1].GetComponentsInChildren<Renderer>();
         renderCorner2_RWB[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[0, 0]));
         renderCorner2_RWB[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[1, 6]));
         renderCorner2_RWB[3].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[2, 2]));

         Renderer[] renderCorner3_RBY = cornerP[2].GetComponentsInChildren<Renderer>();
         renderCorner3_RBY[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[0, 6]));
         renderCorner3_RBY[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[2, 8]));
         renderCorner3_RBY[3].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[3, 0]));

         Renderer[] renderCorner4_BOY = cornerP[3].GetComponentsInChildren<Renderer>();
         renderCorner4_BOY[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[2, 6]));
         renderCorner4_BOY[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[5, 8]));
         renderCorner4_BOY[3].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[3, 6]));

         Renderer[] renderCorner5_WGO = cornerP[4].GetComponentsInChildren<Renderer>();
         renderCorner5_WGO[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[1, 2]));
         renderCorner5_WGO[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[4, 2]));
         renderCorner5_WGO[3].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[5, 0]));

         Renderer[] renderCorner6_OGY = cornerP[5].GetComponentsInChildren<Renderer>();
         renderCorner6_OGY[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[5, 6]));
         renderCorner6_OGY[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[4, 8]));
         renderCorner6_OGY[3].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[3, 8]));    //58

         Renderer[] renderCorner7_RGW = cornerP[6].GetComponentsInChildren<Renderer>();
         renderCorner7_RGW[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[0, 2]));
         renderCorner7_RGW[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[4, 0]));
         renderCorner7_RGW[3].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[1, 8]));

         Renderer[] renderCorner8_RYG = cornerP[7].GetComponentsInChildren<Renderer>();
         renderCorner8_RYG[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[0, 8]));
         renderCorner8_RYG[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[3, 2]));
         renderCorner8_RYG[3].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[4, 6]));


    /*    //BACK-UP EXAMPLE
        Renderer[] renderCorner1_BWO = cornerP[0].GetComponentsInChildren<Renderer>();
        renderCorner1_BWO[1].material.SetColor("_Color", Color.green);
        renderCorner1_BWO[2].material.SetColor("_Color", new Color(1, 0.27f, 0));
        renderCorner1_BWO[3].material.SetColor("_Color", Color.white);

        Renderer[] renderCorner2_RWB = cornerP[1].GetComponentsInChildren<Renderer>();
        renderCorner2_RWB[1].material.SetColor("_Color", Color.yellow);
        renderCorner2_RWB[2].material.SetColor("_Color", Color.green);
        renderCorner2_RWB[3].material.SetColor("_Color", Color.red);

        Renderer[] renderCorner3_RBY = cornerP[2].GetComponentsInChildren<Renderer>();
        renderCorner3_RBY[1].material.SetColor("_Color", Color.red);
        renderCorner3_RBY[2].material.SetColor("_Color", Color.green);
        renderCorner3_RBY[3].material.SetColor("_Color", Color.white);

        Renderer[] renderCorner4_BOY = cornerP[3].GetComponentsInChildren<Renderer>();
        renderCorner4_BOY[1].material.SetColor("_Color", new Color(1, 0.27f, 0));
        renderCorner4_BOY[2].material.SetColor("_Color", Color.green);
        renderCorner4_BOY[3].material.SetColor("_Color", Color.yellow);

        Renderer[] renderCorner5_WGO = cornerP[4].GetComponentsInChildren<Renderer>();
        renderCorner5_WGO[1].material.SetColor("_Color", Color.blue);
        renderCorner5_WGO[2].material.SetColor("_Color", Color.yellow);
        renderCorner5_WGO[3].material.SetColor("_Color", Color.red);

        Renderer[] renderCorner6_OGY = cornerP[5].GetComponentsInChildren<Renderer>();
        renderCorner6_OGY[1].material.SetColor("_Color", Color.white);
        renderCorner6_OGY[2].material.SetColor("_Color", new Color(1, 0.27f, 0));
        renderCorner6_OGY[3].material.SetColor("_Color", Color.blue);    //58

        Renderer[] renderCorner7_RGW = cornerP[6].GetComponentsInChildren<Renderer>();
        renderCorner7_RGW[1].material.SetColor("_Color", Color.blue);
        renderCorner7_RGW[2].material.SetColor("_Color", new Color(1, 0.27f, 0));
        renderCorner7_RGW[3].material.SetColor("_Color", Color.yellow);

        Renderer[] renderCorner8_RYG = cornerP[7].GetComponentsInChildren<Renderer>();
        renderCorner8_RYG[1].material.SetColor("_Color", Color.blue);
        renderCorner8_RYG[2].material.SetColor("_Color", Color.red);
        renderCorner8_RYG[3].material.SetColor("_Color", Color.white);*/
        ...
        for (int i = 0; i < 12; i++)
        {
         ...
         Renderer[] renderEdge1_BW = edgeP[0].GetComponentsInChildren<Renderer>();
         renderEdge1_BW[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[2, 1]));
         renderEdge1_BW[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[1, 3]));

         Renderer[] renderEdge2_YB = edgeP[1].GetComponentsInChildren<Renderer>();
         renderEdge2_YB[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[3, 3]));
         renderEdge2_YB[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[2, 7]));

         Renderer[] renderEdge3_BO = edgeP[2].GetComponentsInChildren<Renderer>();
         renderEdge3_BO[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[2, 3]));
         renderEdge3_BO[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[5, 5]));

         Renderer[] renderEdge4_BR = edgeP[3].GetComponentsInChildren<Renderer>();
         renderEdge4_BR[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[2, 5]));
         renderEdge4_BR[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[0, 3]));

         Renderer[] renderEdge5_WG = edgeP[4].GetComponentsInChildren<Renderer>();
         renderEdge5_WG[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[1, 5]));
         renderEdge5_WG[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[4, 1]));

         Renderer[] renderEdge6_GY = edgeP[5].GetComponentsInChildren<Renderer>();
         renderEdge6_GY[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[4, 7]));
         renderEdge6_GY[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[3, 5]));

         Renderer[] renderEdge7_RG = edgeP[6].GetComponentsInChildren<Renderer>();
         renderEdge7_RG[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[0, 5]));
         renderEdge7_RG[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[4, 3]));

         Renderer[] renderEdge8_OG = edgeP[7].GetComponentsInChildren<Renderer>();
         renderEdge8_OG[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[5, 3]));
         renderEdge8_OG[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[4, 5]));

         Renderer[] renderEdge9_OW = edgeP[8].GetComponentsInChildren<Renderer>();
         renderEdge9_OW[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[5, 1]));
         renderEdge9_OW[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[1, 1]));

         Renderer[] renderEdge10_YO = edgeP[9].GetComponentsInChildren<Renderer>();
         renderEdge10_YO[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[3, 7])); //55
         renderEdge10_YO[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[5, 7]));

         Renderer[] renderEdge11_RY = edgeP[10].GetComponentsInChildren<Renderer>();
         renderEdge11_RY[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[0, 7]));
         renderEdge11_RY[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[3, 1]));

         Renderer[] renderEdge12_RW = edgeP[11].GetComponentsInChildren<Renderer>();
         renderEdge12_RW[1].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[0, 1]));
         renderEdge12_RW[2].material.SetColor("_Color", getColorFromChar(Cube2D.cubeColors[1, 7]));

    /*    //BACK-UP EXAMPLE
        Renderer[] renderEdge1_BW = edgeP[0].GetComponentsInChildren<Renderer>();
        renderEdge1_BW[1].material.SetColor("_Color", Color.blue);
        renderEdge1_BW[2].material.SetColor("_Color", Color.white);

        Renderer[] renderEdge2_YB = edgeP[1].GetComponentsInChildren<Renderer>();
        renderEdge2_YB[1].material.SetColor("_Color", Color.yellow);
        renderEdge2_YB[2].material.SetColor("_Color", Color.red);

        Renderer[] renderEdge3_BO = edgeP[2].GetComponentsInChildren<Renderer>();
        renderEdge3_BO[1].material.SetColor("_Color", new Color(1, 0.27f, 0));
        renderEdge3_BO[2].material.SetColor("_Color", Color.green);

        Renderer[] renderEdge4_BR = edgeP[3].GetComponentsInChildren<Renderer>();
        renderEdge4_BR[1].material.SetColor("_Color", new Color(1, 0.27f, 0));
        renderEdge4_BR[2].material.SetColor("_Color", Color.yellow);

        Renderer[] renderEdge5_WG = edgeP[4].GetComponentsInChildren<Renderer>();
        renderEdge5_WG[1].material.SetColor("_Color", Color.yellow);
        renderEdge5_WG[2].material.SetColor("_Color", Color.blue);

        Renderer[] renderEdge6_GY = edgeP[5].GetComponentsInChildren<Renderer>();
        renderEdge6_GY[1].material.SetColor("_Color", Color.red);
        renderEdge6_GY[2].material.SetColor("_Color", Color.white);

        Renderer[] renderEdge7_RG = edgeP[6].GetComponentsInChildren<Renderer>();
        renderEdge7_RG[1].material.SetColor("_Color", Color.white);
        renderEdge7_RG[2].material.SetColor("_Color", Color.green);

        Renderer[] renderEdge8_OG = edgeP[7].GetComponentsInChildren<Renderer>();
        renderEdge8_OG[1].material.SetColor("_Color", Color.blue);
        renderEdge8_OG[2].material.SetColor("_Color", Color.red);

        Renderer[] renderEdge9_OW = edgeP[8].GetComponentsInChildren<Renderer>();
        renderEdge9_OW[1].material.SetColor("_Color", Color.yellow);
        renderEdge9_OW[2].material.SetColor("_Color", Color.green);

        Renderer[] renderEdge10_YO = edgeP[9].GetComponentsInChildren<Renderer>();
        renderEdge10_YO[1].material.SetColor("_Color", new Color(1, 0.27f, 0)); //55
        renderEdge10_YO[2].material.SetColor("_Color", Color.white);

        Renderer[] renderEdge11_RY = edgeP[10].GetComponentsInChildren<Renderer>();
        renderEdge11_RY[1].material.SetColor("_Color", Color.blue);
        renderEdge11_RY[2].material.SetColor("_Color", new Color(1, 0.27f, 0));

        Renderer[] renderEdge12_RW = edgeP[11].GetComponentsInChildren<Renderer>();
        renderEdge12_RW[1].material.SetColor("_Color", Color.green);
        renderEdge12_RW[2].material.SetColor("_Color", Color.red);*/
    }
    ...
    Color getColorFromChar(char color)
    {
        if (color == 'w')
            return Color.white;
        if (color == 'b')
